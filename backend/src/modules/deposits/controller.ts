...
export async function refundDepositHandler(req: Request, res: Response) {
  ...
  if (!parsed.success) {
    throw new ValidationError("DEPOSIT_REFUND_PAYLOAD_INVALID", "Invalid deposit refund payload", parsed.error.flatten());
  }
...
export async function adjustDepositHandler(req: Request, res: Response) {
  ...
  if (!parsed.success) {
    throw new ValidationError("DEPOSIT_ADJUSTMENT_PAYLOAD_INVALID", "Invalid deposit adjustment payload", parsed.error.flatten());
  }
